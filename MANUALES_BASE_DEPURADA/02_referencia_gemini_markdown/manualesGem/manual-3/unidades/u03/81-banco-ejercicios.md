...
> Banco de práctica de prompt engineering. Privilegia ejercicios de **redacción**, **evaluación de pares prompt-respuesta** y **detección de antipatrones**. Trabaja en tu plataforma base (la que elegiste en U02). Resuelve antes de mirar la clave.
...
Para cada tarea, elige la técnica más eficiente (zero-shot, few-shot, chain-of-thought, self-consistency) y justifica:
...
1. NO uses CoT cuando:
   ...
   - [x] La tarea es trivial (traducción palabra-palabra) y solo agregas latencia y costo sin ganar precisión
   - [ ] Quieres trazabilidad del razonamiento

2. Few-shot con 50 ejemplos:
   - [ ] Es siempre mejor que con 5
   - [ ] No tiene problema de costo
   - [x] Aumenta costo, latencia y riesgo de "lost-in-the-middle"; el sweet spot suele ser 3-5 ejemplos
...
Quiero que clasifiques 10 correos. Devuelve la respuesta en formato _____________ con los siguientes campos por correo: id, categoria (una de [familia, escuela, trabajo, suscripcion, basura]), confianza (un número entre _____________ y _____________), y razonamiento_corto (máximo _____________ palabras). NO incluyas texto adicional fuera del _____________; si la confianza es menor a _____________, marca categoria como _____________.
::/act-fill
...
Para cada prompt, identifica el antipatrón principal y propón una versión mejorada:
...
1. ¿Cuál de estos es el antipatrón **más dañino** porque es el más invisible?
   ...
   - [ ] Pedir formato JSON
   - [x] Mezclar 2 instrucciones contradictorias en el prompt ("sé conciso pero exhaustivo")
   - [ ] No definir rol
...
**Bloque A — Fill (ejemplo válido):** 1) "Eres tutor de Física para estudiante de prepa, paciente y socrático" · 2) "Explícame el principio de Bernoulli" · 3) "Soy estudiante de 4to año, examen el viernes, manejo derivadas básicas pero me cuestan los problemas con dimensiones reales" · 4) "Máximo 600 palabras, español neutro, sin jerga universitaria" · 5) "Estructura: intuición física + fórmula + ejemplo numérico con valores reales + 1 pregunta socrática" · 6) "Estilo similar al de Walter Lewin en sus clases de MIT". **MCQ:** 1-c · 2-c.

**Bloque B — Caso:** 1) zero-shot. 2) chain-of-thought (matemática). 3) few-shot con 5 ejemplos custom. 4) self-consistency (ejecutar varias veces y consensuar) o CoT con verificación. 5) zero-shot (creativo, no determinista). **MCQ:** 1-c · 2-c.

**Bloque C — Match:** 1-d · 2-a · 3-c · 4-b · 5-e. **Fill:** JSON · 0 · 1 · 20 · JSON · 0.6 · "incierto".

**Bloque D — Order:** v1 → ejecuta → observa → compara contra criterios → diagnostica capa → modifica solo esa capa → re-ejecuta y mide → guarda en banco. **V/F:** 1-F (es estimado) · 2-F (es la práctica profesional) · 3-V · 4-F (no aprendes qué cambió) · 5-V.

**Bloque E — Caso:** 1) Vaguedad ("buen") → especificar género, longitud, nivel, criterios. 2) Restricciones contradictorias → separar en 3 prompts. 3) Pregunta sin criterio objetivo → reformular como "según índices de gobernanza X y Y". 4) Sobrecarga + lost-in-middle → fragmentar en pipeline de 4 prompts. **MCQ:** 1-c · 2-c.
