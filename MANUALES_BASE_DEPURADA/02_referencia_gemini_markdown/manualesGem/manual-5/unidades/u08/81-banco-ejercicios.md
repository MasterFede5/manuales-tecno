...
Tu app de predictor llevaba 3 días en producción. Hoy llega la factura: USD 240. Esperabas USD 20.
...
- `act-case`: respuesta modelo:
  > "(1) Loop infinito que llama API; max_tokens muy alto; usuarios bot/scrapers; sin caching de queries repetidas; modelo sobredimensionado para la tarea. (2) Detectar antes: dashboard de gasto en consola del proveedor con alertas; logs por request con costo estimado. (3) Controles: rate limit por IP (10 req/min), presupuesto diario tope (USD 5/día), alerta a Slack si gasto >50% del tope antes del fin de día."

> **Cierre.** Si dominas estos 7 bloques, tu app final cumple los 5 requisitos profesionales: segura, robusta, eficiente, observable, sostenible en costo.
