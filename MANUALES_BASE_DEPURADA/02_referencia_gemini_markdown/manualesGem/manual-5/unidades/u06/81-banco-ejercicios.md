...
> Practica clustering, PCA y detección de anomalías. Cada bloque conecta con el case escolar.
...
- `act-case`: respuesta modelo:
  > "(1) K=4 fue donde el codo se aplanó, silueta dio 0.42, y los 4 perfiles tienen sentido pedagógico. (2) Validar: re-correr con `random_state` distintos; si los clusters son robustos, las composiciones cambian poco. Adicional: comparar contra K=3 y K=5 para ver si los nuevos clusters son sub-divisiones útiles. (3) Re-correr el clustering bimestre a bimestre y medir cuántos alumnos cambian de cluster. Si <20% migran, los clusters son estables; si >50%, son volátiles."

> **Cierre.** El clustering no devuelve verdades absolutas: devuelve **hipótesis estructurales** que validas con dominio y tiempo.
