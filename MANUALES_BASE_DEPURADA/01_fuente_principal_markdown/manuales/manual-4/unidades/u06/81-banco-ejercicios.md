...
> Trabajas con n8n AI Agent o Claude API directa. 
> Tienes 3 tools básicos ya construidos en U5:
> - RAG
> - Calendario
> - Ticket Notion

::interioriza
Imagina un agente como un conserje inteligente de la escuela. No solo tiene las llaves (tools) de cada salón, sino que sabe en qué orden usarlas para resolver un problema de aulas, adaptándose a los obstáculos que encuentre.
::/interioriza

::pausa{}
¿Si el conserje no tuviera memoria de qué llaves ya probó, qué pasaría al intentar abrir una puerta atascada?
::/pausa
...
Tu agente necesita poder enviar correos institucionales. 
Define el tool completo detallando:
- Nombre y descripción.
- Schema JSON.
- Antiejemplos (cuándo NO usar).
- Efectos colaterales y si requiere approval.
- Whitelist de dominios y plantilla de mensaje.
::/act-case
...
Tu agente lleva 1 semana en producción. Recibes alerta crítica: gastó $40 USD en 3 horas (presupuesto diario $50). 
Logs muestran: el agente entró en loop intentando crear un ticket que ya existía (idempotency falló por coma extra en el email). 

Diseña tu respuesta:
1. Mitigación inmediata.
2. Qué buscas en logs.
3. Cómo restauras esta noche.
4. 3 cambios estructurales para que no vuelva a pasar.
...
**B2.** 
- **1) Falso:** Multi-agente añade complejidad y costo; conviene cuando hay perspectivas divergentes.
- **2) Falso:** Más tools dispersan la atención del modelo y aumentan riesgo de error.
- **3) Falso:** Incluso un agente reactivo necesita memoria de la última acción para no repetir.
- **4) Verdadero (generalmente):** Más iteraciones = más tokens, pero **resuelve casos** que el reactivo no puede.

**C1.** Sugerencia (acepta variantes razonables): 
- **No técnicos:** Lindy o ChatGPT Agents.
- **Browser:** Browserbase, Multi-On, ChatGPT Agents.
- **Computadora:** Claude Computer Use o Manus.
- **n8n integrado:** n8n AI Agent node.
- **Software:** Devin, Cursor agents, GitHub Copilot Workspace.
- **Razonamiento profundo + multi-tool:** Claude Opus + function calling vía SDK.
...
**E2.** 
- **1) Falso:** System prompt es necesario pero no suficiente; un usuario hostil bypassa los prompts.
- **2) Falso:** 50 iters es ventana enorme para gastar mucho; defaults sanos: 8-15.
- **3) Falso:** Approval para **todo** anula el agente; aprueba solo lo destructivo o costoso.
- **4) Falso:** Logging es ~3-5 % del costo, irrelevante; no loguear es la causa #1 de incidentes sin diagnóstico.

**E3.** Respuesta libre. Buena respuesta: 
- **(1)** Corto el agente con kill-switch en n8n.
- **(2)** Busco en logs último thought antes del loop y el error de idempotency check.
- **(3)** Restauro con max_iterations bajado a 5 temporalmente.
- **(4)** Cambios estructurales: normalizador de email + idempotency con hash + alerta por retries.
- **(5)** Comunico el incidente y plan en `#alertas-asistente`.
