...
::concepto
- Banco de práctica para Model Context Protocol (MCP).
- Cubre: arquitectura cliente-servidor, catálogo, Claude Desktop, ChatGPT Connectors y casos de uso.
- Privilegia **decisiones de servidor**, **lectura de config** y **diseño de flujo**. 
- ¡Resuelve antes de mirar la clave!
::/concepto

::interioriza
Piensa en MCP como un enchufe universal.
- Antes, cada electrodoméstico necesitaba un tipo de enchufe diferente.
- Ahora, con MCP, tu asistente de IA usa el mismo enchufe para conectarse a cualquier "electrodoméstico" institucional (Slack, GitHub, BD).
::/interioriza

::pausa{}
Antes de iniciar los ejercicios, pregúntate: ¿Por qué es vital estandarizar la conexión entre LLMs y herramientas locales?
::/pausa
...
1. **_____________**: Funciones invocables que producen efecto en el mundo. Ejemplos: `commit_git`, `send_slack_message`.
2. **_____________**: Contenido leíble, estado del mundo. Ejemplos: `file://path`, `notion://page/{id}`.
3. **_____________**: Plantillas reutilizables. Ejemplo: "resumir reunión usando contexto de [resource]".

Dinámica del cliente LLM:
- Descubre capacidades con `_____________` y `_____________`.
- Decide qué invocar y maneja la respuesta.
- MCP fue lanzado por _____________ en _____________ de 2024.
- En 2025 lo adoptaron OpenAI, Cursor, Windsurf y otros.
::/act-fill
...
- **ChatGPT con _____________**: Tareas mainstream (Drive, Calendar, Slack) con OAuth y _____________ enterprise grade.
- **Claude Desktop con _____________**: Tareas custom internas, servidores MCP locales y máxima _____________.

Anti-patrón: 
- Configurar los _____________ OAuth tokens en ambos clientes. 
- Replicas la exposición y la auditoría se vuelve confusa. 
- Cada usuario tiene ambos clientes y el gasto se distribuye.
::/act-fill
...
Diseña un **6° caso de uso real** específico para tu institución. Debe incluir:
- a) **Dolor concreto** que resuelve (ej: 3 h semanales cruzando pagos).
- b) **2-4 servidores MCP** combinados.
- c) **Flujo paso a paso** (qué tool invoca el modelo, en qué orden).
- d) **Dónde aplica approval flow** (acciones irreversibles).
- e) **Tiempo manual ahorrado** estimado vs. tiempo con MCP.
::/act-case
