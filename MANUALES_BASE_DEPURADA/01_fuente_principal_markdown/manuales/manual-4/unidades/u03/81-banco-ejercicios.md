...
> Trabajas con Claude.ai abierto y un editor markdown a la mano.
> Cada ejercicio puede pedir que **publiques un Artifact real** (gratis o demo).

::interioriza
Resolver estos ejercicios es como armar un set de Lego:
- Clasificas las piezas (eliges el proveedor adecuado).
- Armas la base estructural (primer prompt estructurado).
- Añades los detalles finales (iteración y polish visual).
::/interioriza

::pausa{titulo="Reflexión antes de empezar"}
Antes de escribir tu primer prompt, pregúntate:
¿Mi caso de uso realmente no requiere una base de datos o backend complejo?
Si la respuesta es sí, ¡un Artifact es el camino ideal!
::/pausa
...
Completa la tabla con tu **percepción operativa actual**:
(No es absoluta — actualízala cuando los productos evolucionen)
...
Tu institución tiene 12 grupos de bachillerato.
La coordinadora pierde 30 min cada mañana actualizando una hoja Excel mental.

Necesitas un Artifact que:
- Muestre los 12 grupos en cards.
- Tenga cada card con cupo total, ocupados y disponibles.
- Incluya un botón "+1 inscrito" y "-1 baja".
- Muestre un total general arriba.
- Lance una alerta visual cuando un grupo queda con menos de 3 cupos.

Datos del Asistente: el director firmó política de "cupos visibles a todos".
Escribe el primer prompt completo (las 6 dimensiones).
::/act-case
...
Tu suite institucional está creciendo:
- Dashboard, calculadora cupos.
- Generador comunicados, simulador horarios.
- Quiz vocacional, pre-inscripción.
Son 6 Artifacts en total.

La coordinadora pregunta: *"¿Podemos tenerlos todos en un mismo lugar para no perder links?"*.

Diseña tu solución:
- ¿Un hub Artifact?
- ¿Una página Notion?
- ¿Una landing en Vercel?
- ¿Una app real?

Justifica tu decisión con costo (tiempo/dinero) y riesgo.
::/act-case

::act-mindmap{titulo="F2. Tu suite institucional v1" centro="SUITE ALBATROS v1.0" nodos_primarios=6 nodos_secundarios=12}
Cada rama es una mini-app.
Para cada mini-app define:
- Nombre, audiencia y métrica única de uso.
- Versión actual y último cambio.
- Próximo issue o mejora planeada.
::/act-mindmap
...
**A2.**
- 1) Falso — Artifacts tienen sandbox; las llamadas a APIs externas suelen estar restringidas.
- 2) Falso — sin backend no hay persistencia compartida; migra a deploy real.
- 3) Falso — los links públicos pueden ser indexables o no, depende del proveedor.
- 4) Falso — son útiles en sesión pero no sustituyen Git. El día que caduca tu cuenta, pierdes todo.

**B1.** 1-b · 2-c · 3-d · 4-a.

**B2.** Sugerencia (acepta variantes razonables y actualízala):
- Claude: mejor React, link público nativo, mobile bien.
- ChatGPT: mejor docs largos y Python con Code Interpreter.
- Gemini: mejor integración Workspace.

**C1.** Propósito · Componentes · Datos · Estilo · Interacciones · Restricciones.

**C2.** Respuesta libre. Verifica que los 6 elementos estén explícitos. Buen prompt:
*"Propósito: ayudar a la coordinadora ver y actualizar cupos de 12 grupos.*
*Componentes: 4x3 grid de cards + total arriba.*
*Datos: 12 grupos (1A-6A, 1B-6B), cada uno con cupo total entre 25-30.*
*Estilo: paleta azul Albatros #0E3A8A, acento naranja #F39C12, Inter 14px.*
*Interacciones: botón +1/-1 por card, alerta roja cuando disponibles<3.*
*Restricciones: sin backend, sin APIs externas, datos en estado local; mobile-first; sin emojis."*
...
**E2.**
- 1) Falso — el link depende del proveedor; sin código en Git, te quedas sin Artifact.
- 2) Falso — los snapshots son evidencia visual de estilos propietarios.
- 3) Falso — typos van a parche x.x.+1, paletas pueden ser minor x.+1.0. Sigue semver.
- 4) Verdadero — un buen CHANGELOG documenta qué se intentó y descartó.

**F1.** Respuesta libre.
Buena respuesta defiende:
- Hasta 4-5 Artifacts → hub HTML simple basta.
- >5 o necesidad de auth/persistencia → Notion público o landing Vercel.
- Necesidad de control multi-usuario → app real con backend.
