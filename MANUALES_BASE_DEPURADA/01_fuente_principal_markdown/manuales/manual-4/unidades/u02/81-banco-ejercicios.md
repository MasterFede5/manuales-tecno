...
> - Cada ejercicio entrena un movimiento concreto del PRD.
> - Trabajas con tu repo de la U1 abierto.
> - Usa un editor markdown a la mano.

::interioriza
Imagina que este banco de ejercicios es el simulador de vuelo de tu asistente escolar.
Aquí puedes equivocarte sin consecuencias antes de empezar a programar.
::/interioriza

::pausa{}
**Deduce:**
- ¿Por qué es vital tener el repo de la U1 abierto mientras resuelves esto?
- ¿Qué pasaría si te saltas la planeación y vas directo al código?
::/pausa

### Sección A — PRD asistido por IA (2.1)

::act-fill{titulo="A1. Las 8 secciones obligatorias del PRD"}
- Llena los huecos para completar el estándar.
- Un PRD de 1 página exige **siempre** estas 8 secciones en orden:
...
- Tu director firma: *"el Asistente debe mejorar la atención a familias"*.
- Reescribe esa frase como **3 métricas verificables** (numéricas, con cómo se mide y umbral).
- Mínimo una métrica de calidad, una de eficiencia, una de cobertura.
::/act-case

### Sección B — Plantillas Albatros: brief, scope, criterios (2.2)

::interioriza
Un Brief es como la receta de cocina para un módulo específico.
El PRD, en cambio, es el menú completo de todo el restaurante.
::/interioriza

::act-fill{titulo="B1. Brief mínimo de un módulo"}
- Un brief no es un PRD: es una orden de trabajo para **un módulo**.
- Sus 7 campos obligatorios son:
...
- Ordena los 9 ítems del DoD canónico (de la U2.3).
- Hazlo según su momento de aplicación:
...
- Tu spec dice: *"el clasificador devuelve `categoria_principal` (1 de 5 valores), `confianza` (0-1) y `etiquetas_secundarias` (array máx. 3). Si confianza < 0.7, agrega `requiere_revision_humana=true`."*
- Escribe el prompt XML que materializa la spec **sin desviarse**.
- Deja claro qué prueba va a confirmar que prompt y spec siguen sincronizados.
::/act-case
...
- Completa la tabla con el rango ±% típico de incertidumbre.
- Añade la acción concreta para cerrar el cono:
...
- Tu director propone usar GPT-5 (calidad alta, $0.04/llamada) para todo.
- La coordinadora prefiere Claude Haiku (calidad media, $0.004/llamada).
- El presupuesto de operación anual es $2 400. Volumen estimado: 60 mil llamadas/año.

- Escribe el **ADR-001** con: contexto · decisión · alternativas · consecuencias · revisión.
- Argumenta con números, no con preferencias.
::/act-case
...
**A3.** Sugerencia (acepta variantes razonables): 
- Calidad: *"satisfacción declarada por familia ≥ 4.2/5 en encuesta post-respuesta, n≥30/mes"*.
- Eficiencia: *"tiempo mediano de primera respuesta < 2 horas en 90 % de los casos"*.
- Cobertura: *"≥ 70 % de respuestas resueltas sin escalación a humano"*.

**B1.** Título · Contexto · Usuario · Alcance · Criterios · Métrica única · Riesgo principal.

**B2.** Sugerencia parcial: 
- Clasificador IN: 5 categorías + confianza + etiquetas. 
- OUT: respuestas, integración n8n, multi-idioma. 
- Criterio principal: macro-F1 ≥ 0.85 sobre golden 50.

**C1.** Orden propuesto: 
Métrica única → Schema validado → Tests unitarios → Smoke test → Golden set → Costo y latencia → Plan de rollback → Documentación → Approval.

**C2.** 
- 1) Falso: golden es **una** condición; faltan rollback, costo medido, approval, etc. 
- 2) Falso: el DoD canónico es base; cada módulo suma ítems propios (ej. revisión legal). 
- 3) Falso: el approval no es prueba técnica, es validación social contra retrabajos.

**D1.** 1-c · 2-a · 3-b · 4-d.

**D2.** S-001 · T-027 · F-014 · O-003.

**E1.** Respuesta libre. Verifica: 
- (a) prompt usa los nombres exactos de campos del schema.
- (b) la regla de `requiere_revision_humana` está explícita en `<constraints>`.
- (c) la prueba es **golden con validación JSON Schema** + test de exclusión de campos extra.

**E2.** c.

**F1.** Sugerencia: 
- Idea inicial ±300 % (entrevistar stakeholders) 
- Discovery ±100 % (PRD borrador) 
- PRD aprobado ±50 % (briefs) 
- Briefs ±25 % (smoke test) 
- Smoke ±10 % (golden completo).

**F2.** Respuesta libre. Una buena respuesta calcula: 
- 60k × $0.04 = $2 400 (consume **todo** el presupuesto). 
- 60k × $0.004 = $240 (deja $2 160 para imprevistos). 
- Decisión: Haiku por defecto, escalar a Sonnet/GPT si confianza < 0.7. Revisión a 90 días.
