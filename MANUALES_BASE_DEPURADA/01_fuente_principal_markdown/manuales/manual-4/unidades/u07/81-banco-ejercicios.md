...
> Trabajas con tu computadora (Linux, Mac o Windows con WSL). 
> - Tienes Ollama instalado y al menos 16 GB de RAM. 
> - Si no tienes GPU, igual puedes hacer la mayoría de los ejercicios con modelos pequeños (Qwen 2.5 7B, Phi-3).

::interioriza
Imagina que los LLMs son como vehículos:
- Modelos pesados (70B) son como un tráiler: necesitan un motor potente (mucha VRAM).
- Modelos pequeños (7B) son como un auto compacto: corren muy bien solo con tu RAM normal.
::

::pausa{titulo="Antes de empezar: deducción rápida"}
1. Si un modelo no cabe en tu VRAM, ¿qué componente de tu PC entrará al rescate (y hará la inferencia más lenta)?
2. ¿Por qué una escuela preferiría "comprar el tráiler" (local) en lugar de rentarlo (usar Cloud)?
::
...
Q4_K_M significa modelo cuantizado a _____________ bits con _____________ calidad ~98 %, _____________ memoria. 
Q8 mantiene calidad casi sin pérdida pero usa _____________ memoria que Q4. 
Q2 es la **menor** memoria pero pierde mucha _____________. 
El "sweet spot" calidad/tamaño suele ser _____________.
::/act-fill
...
Tu institución gastará $4 200/año en cloud (Claude Sonnet). Para migrar a local, propones:
- Workstation $3 500 (one-time)
- Electricidad $80/mes
- Mantenimiento 4 h/mes a $25/h

Calcula TCO a 12, 24 y 36 meses comparando ambos. ¿En qué momento local "paga"? 
¿Qué consideraciones no-monetarias importan más que el costo?
::/act-case
...
**A2.** 
- 1) Falso — local tiene costo de hardware, electricidad, mantenimiento; cloud puede ser más barato a volúmenes bajos. 
- 2) Falso — CPU corre modelos pero a 10x menor tokens/s; para producción se recomienda GPU o Apple Silicon. 
- 3) Falso — local mitiga riesgo de leak con proveedor pero introduce riesgos de fuga interna; sin política de acceso, los datos siguen vulnerables. 
- 4) Generalmente cierto — Qwen 2.5 72B compite cerca de Sonnet en español; valida con tu golden set.
...
- Laptop 16 GB → Qwen 2.5 7B Q4 (CPU inference) para personal/dev. 
- RTX 3060 12 GB → Qwen 2.5 14B Q4 o Llama 3.1 8B Q8 para single user. 
- A6000 48 GB → Qwen 2.5 72B Q4_K_M para multi-usuario institucional. 
- Mac M3 Max 128 GB → Llama 3.1 70B Q8 o Qwen 2.5 72B Q8 con buen rendimiento. 
- Servidor multi-GPU → modelos 100B+ o múltiples instancias balanceadas.
...
**E1.** 
- 1) Falso — el embedder cloud envía cada chunk a OpenAI; los datos salen. 
- 2) Verdadero. 
- 3) Verdadero.
