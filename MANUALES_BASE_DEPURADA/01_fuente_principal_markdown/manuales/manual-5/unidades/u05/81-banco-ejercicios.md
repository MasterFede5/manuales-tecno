...
> Practica el pipeline ML:
> - Features
> - Split
> - Entrenar
> - Evaluar
> 
> Cada bloque te pide leer código `sklearn` y predecir métricas.

::interioriza{titulo="Analogía: El Chef ML"}
El pipeline ML es como preparar una receta:
- **Features:** Ingredientes limpios y listos.
- **Split:** Apartar una porción intacta para el jurado final.
- **Entrenar:** Cocinar el platillo y ajustar la sazón.
- **Evaluar:** El jurado prueba tu platillo.
::/interioriza

::pausa{titulo="Reflexión inicial"}
- ¿Por qué no probarías la receta final con la misma porción que preparaste mientras ajustabas la sazón?
- *(Pista: Evitar el Data Leakage y medir generalización)*.
::/pausa
...
**Pregunta.**
- Si **interpretabilidad** es prioridad para coordinación, ¿cuál eliges?
- Si **precisión** es prioridad y aceptas algo de overfit, ¿cuál?
- Si quieres **balance**, ¿cuál eliges?
*(Justifica cada elección en 1 línea)*.
::/act-case
...
- `act-case`: respuesta modelo:
  > - **Interpretabilidad:** Lineal (ecuación clara, gap mínimo).
  > - **Precisión:** Random Forest (overfit moderado, no catastrófico).
  > - **Balance:** Árbol depth=4 (mejora sobre lineal sin overfit grave; visual).

> **Cierre.**
> Si dominas la tabla del bloque G y las métricas del bloque D:
> ¡Estás listo para el taller (clasificación binaria) y la implementación!
