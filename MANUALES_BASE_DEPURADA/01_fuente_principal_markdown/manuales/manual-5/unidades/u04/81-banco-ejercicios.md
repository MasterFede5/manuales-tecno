...
- **Enfoque Práctico:** Estadística aplicada con código real.
- **Tu Tarea:** Leer outputs y predecir los resultados correctos.

::interioriza
Imagina que la estadística es como un colador de cocina.
Al pasar los datos por él, separas la información útil (la pasta) del ruido (el agua).
¡Vamos a usar nuestro colador!
::

::pausa{}
**¿Qué buscamos lograr al predecir resultados?**
- A) Memorizar código de Python.
- B) Entender la lógica subyacente de la estadística aplicada.
*(Piensa tu respuesta antes de continuar)*
::

### Bloque A — Tendencia central y dispersión (4.1)

- **Objetivo:** Identificar dónde se concentran los datos.
- **Reto:** Diferenciar entre media, mediana y moda en casos reales.
...
- **Concepto clave:** La campana de Gauss.
- **Importancia:** Permite predecir probabilidades usando la regla empírica.

::interioriza
Piensa en la distribución normal como las estaturas de una clase.
La mayoría está cerca del promedio (en el centro).
Muy pocos son extremadamente altos o bajos (en los extremos).
::

::pausa{}
**Si la mayoría de las notas de un examen están en 7...**
¿Dónde esperarías ver el pico de la campana?
*(Respuesta: Exactamente en el 7).*
::
...
- **Posición relativa:** Los percentiles dividen los datos en 100 partes.
- **Cuartiles:** Son los percentiles 25, 50 y 75.
...
- **Fuerza y Dirección:** Mide qué tan juntas se mueven dos variables.
- **Rango:** Varía entre -1 y 1.

::interioriza
La correlación de Pearson es como un baile en pareja.
Si uno da un paso adelante y el otro también, la correlación es positiva (r cercano a 1).
Si uno avanza y el otro retrocede, es negativa (r cercano a -1).
::
...
- **Confounder:** Una tercera variable que afecta a ambas.
...
- **Datos atípicos:** Valores extremos que se alejan del patrón.
- **Método robusto:** Usar el Rango Intercuartílico (IQR).
...
- **Práctica final:** Integra todos los conceptos aprendidos.
- **Decisión basada en datos:** Aprende a justificar afirmaciones.
